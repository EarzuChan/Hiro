...
1. 由于这走的是自提供的 Skia，打包相应的 `.so`（动态共享库）与相关资源不可避，这将导致您的安装包增大 6MB，我已竭尽全力减小裁剪以缩减体积。
...
- 我根据 AOSP Material 3 水波纹动画的底层原理，使用 Skiko 图形栈复刻了该效果。这使得在低版本 Android、魔改 ROM 上也能有一致的 Material 3 水波纹体验。这个复刻已内含在 Hiro Material 3 中，我或许之后会另外贡献给上游CMP
...
- Hiro Skia 基本上是为 Hiro Compose 打造的。我没有直接依赖存在已知 Bug（比如把 `makeWH` 传成了 `width,width`）的 JetBrains 官方安卓示例，而是实现了一套轻量级的 Skia 的 Layer 和 SurfaceView，对于 Hiro Compose 的显示是基本够用，而独立的 Skia 承载用途，则未经深度测试
