...
abstract class HiroSkikoVariantRule : ComponentMetadataRule {
    ...
    private fun ComponentMetadataDetails.blockComposeModuleFromHiroClasspath() {
        allVariants(object : Action<VariantMetadata> {
            override fun execute(variant: VariantMetadata) {
                variant.attributes(object : Action<AttributeContainer> {
                    override fun execute(attributes: AttributeContainer) {
                        attributes.attribute(HiroAttributes.skikoBackend, HiroAttributes.blockedCompose)
                    }
                })
            }
        })
    ...
    private companion object {
        ...
        val blockComposeDependenciesAction =
            object : Action<DirectDependenciesMetadata> {
                override fun execute(dependencies: DirectDependenciesMetadata) {
                    HiroComposeDependencyBlocker.removeComposeDependencies(dependencies)
                }
            }
    }
}
