...
flowchart TD
    ...
    B --> Q["HiroSkikoComposeScene"]
    Q --> J["CanvasLayersComposeScene"]
    J --> I["ComposeSceneRecomposer"]
...
Skiko 请求 frame
  -> SkikoRenderDelegate.onRender(skiaCanvas, width, height, nanoTime)
    -> HiroSkikoComposeScene.render(skiaCanvas, width, height, nanoTime)
    -> skiaCanvas.asComposeCanvas()
    -> CanvasLayersComposeScene.render(composeCanvas, nanoTime)
    -> ComposeSceneRecomposer / measure / layout / draw
    -> SkiaBackedCanvas
    -> Skia flush / GL present
```

CMP `1.11.1`已发布 artifact 里的实际复用对象是：

```text
CanvasLayersComposeScene
ComposeScene.render(canvas, nanoTime)
ComposeSceneRecomposer
SkiaBackedCanvas / asComposeCanvas
```

其中 `FrameRecomposer` / `SingleComposeSceneRenderingScope` 只出现在更后的 CMP 源码线上，不作为 `1.11.1` 阶段的 P0.3 目标。等后续 CMP stable 正式发布这些 API，再评估是否切换。
...
- 持有 `SkiaLayer`；
- 持有 `HiroSkikoComposeScene`；
- 间接持有 `CanvasLayersComposeScene`；
- 通过 `ComposeScene.render` 驱动 CMP `1.11.1` 内部的 `ComposeSceneRecomposer`；
...
- 让 Android 产物接入 CMP `1.11.1` 的 common/skiko 窗口无关实现，并在 Hiro 侧提供 Android-Skiko 原型封装；
- 跑通 `CanvasLayersComposeScene`；
- 跑通 `ComposeSceneRecomposer + ComposeScene.render(canvas, nanoTime)`；
- 跑通 `SkiaBackedCanvas`。

### P0.4 实现 `HiroComposeView`

- `setContent {}`，以及给Activity搞扩展方法直接附加布局并真setContent
- attach/detach；
- schedule frame；
- size/density/layout direction；
- render delegate；
